...
import { FaHome, FaSearch, FaTruck, FaHistory, FaChevronLeft, FaChevronRight } from 'react-icons/fa';

const DistributorSidebar = ({ onCollapse }) => {
    ...
    const menuItems = [
        { path: '/distributor/home', icon: <FaHome />, label: 'Home' },
        { path: '/distributor/food-near-me', icon: <FaSearch />, label: 'Food Near Me' },
        { path: '/distributor/tracking', icon: <FaTruck />, label: 'Tracking' },
        { path: '/distributor/history', icon: <FaHistory />, label: 'History' }
    ];
            ...
            <div className="absolute bottom-0 w-full p-4 bg-indigo-900">
                <div className="flex items-center">
                    <div className="w-10 h-10 rounded-full bg-emerald-400 flex items-center justify-center transition-transform duration-300 hover:scale-110">
                        <span className="text-indigo-950 font-bold">D</span>
                    </div>
                    {!isCollapsed && (
                        <div className="ml-4">
                            <p className="text-sm font-medium text-white">Distributor</p>
                            <p className="text-xs text-indigo-300">NGO</p>
                        </div>
                    )}
                </div>
            </div>
